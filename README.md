...
本项目已扩展为 30 条测试用例，覆盖产品定义、流程机制、知识库命中、站内实时补充、跨概念对比、合规风险和无答案拒答等场景。
...
1. **知识库召回不稳定**  
   部分产品定义、四层链路、评估指标等知识块没有稳定命中。

2. **判断节点误判**  
   判断节点存在过严或过宽问题，可能把可回答问题判为 NO，或把泛相关内容判为 YES。

3. **sitemap 实时补充不稳定**  
   部分站内文章没有被 sitemap URL 选择节点正确定位。

4. **回答结构不够贴合问题**  
   长文章命中后，回答有时先展开细节，没有先直接回答用户核心问题。

5. **合规风险**  
   平台违规、账号封禁、风控类问题中，模型可能给出规避平台规则的建议；其中 T29 被标记为 P0，需要优先修复。
...
- 设计 30 条测试用例，覆盖产品定义、流程机制、知识库命中、站内实时补充、跨概念对比、合规风险和无答案拒答等场景。
